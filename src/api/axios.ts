...
if (USE_MOCKS) {
  ...
  const mockAdapter = async (config: InternalAxiosRequestConfig): Promise<any> => {
    ...
    if (url.includes('/chat') && method === 'POST') {
      ...
      return {
        data: {
          assistant: `Echo: ${userMessage}\n\nThis is a mock response from the assistant. In a real application, this would be generated by an LLM. The message has been received and processed successfully.`,
          conversationId: data.conversationId || 'c_001',
          canvasContent: `# Canvas Content\n\nGenerated from: "${userMessage}"\n\n## Sample Code\n\n\`\`\`javascript\nconst result = processMessage("${userMessage}");\nconsole.log(result);\n\`\`\`\n\n## Notes\n\n- This is sample canvas content\n- It can be applied to the canvas panel\n- It demonstrates the integration`,
        },
        status: 200,
        statusText: 'OK',
        headers: {},
